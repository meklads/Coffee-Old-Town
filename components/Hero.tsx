...
const Hero: React.FC = () => {
  ...
  return (
    <section id={SectionId.PHASE_01_SCAN} className="relative min-h-screen bg-brand-light flex items-center overflow-hidden select-none pt-24 pb-12">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-[radial-gradient(circle_at_center,_rgba(194,163,107,0.04)_0%,_transparent_85%)] pointer-events-none" />

      <div className="max-w-7xl w-full mx-auto px-6 z-10">
        
        {/* Main 2-Column Grid Split (Left Cluster vs Right Scanner) */}
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-24 items-center">
          
          {/* LEFT SIDE: Heading + Archive Cluster */}
          <div className="flex flex-col space-y-8 lg:space-y-10">
            ...
            <div className="space-y-6">
               ...
               <div className="text-left">
                 <h1 className="text-6xl md:text-8xl lg:text-[100px] font-serif font-bold text-brand-dark leading-[0.8] tracking-tighter">
                 ...
                 <p className="text-brand-dark/30 text-[10px] md:text-[11px] font-bold tracking-tight mt-6 ml-1 max-w-sm">
                    Precision instrumentation for biological synthesis and metabolic optimization.
                 </p>
               </div>
            </div>

            {/* 2. Archive Module (Directly below Heading) */}
            <div className="relative group w-full aspect-[16/9.5] overflow-hidden rounded-[48px] bg-brand-sand/5 border border-brand-dark/[0.04] shadow-sm">
               {/* Telemetry Float Overlay inside the box for cohesion */}
               <div className="absolute top-8 left-8 flex gap-6 z-20 opacity-40">
               ...
               {galleryItems.map((item, idx) => (
                 ...
                 >
                   <img src={item.img} className="w-full h-full object-cover saturate-[0.4] contrast-[1.05]" alt="Archive Specimen" />
          ...
          {/* RIGHT SIDE: Scanner Module (Centered Vertically) */}
          <div className="flex justify-center lg:justify-end">
             <div 
               className="relative w-full max-w-[340px] md:max-w-[400px] transition-all duration-1000"
               style={{ transform: `translate(${mousePos.x}px, ${mousePos.y}px)` }}
             >
               {/* Label Badge */}
               <div className="absolute -top-10 right-6 flex items-center gap-3 opacity-30">
               ...
               <div 
                 ...
                 className="relative aspect-[3/4] rounded-[64px] border border-brand-dark/[0.08] bg-white overflow-hidden cursor-pointer flex flex-col items-center justify-center transition-all group shadow-sm hover:shadow-2xl"
               >
                  {/* Digital Brackets */}
                  <div className="absolute top-0 left-0 w-24 h-24 border-t-2 border-l-2 border-brand-primary/10 rounded-tl-[64px]" />
                  <div className="absolute bottom-0 right-0 w-24 h-24 border-b-2 border-r-2 border-brand-primary/10 rounded-br-[64px]" />
                  ...
                  {image ? (
                    <div className="absolute inset-0">
                      ...
                      <div className="absolute inset-0 bg-brand-dark/90 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center gap-8">
                         <div className="relative">
                            ...
                            <button onClick={(e) => { e.stopPropagation(); handleAnalyze(); }} className="relative bg-brand-primary text-white p-7 rounded-full shadow-3xl">
                         ...
                         <span className="text-[9px] font-black text-white tracking-[0.6em] animate-pulse">INITIATE_DIAGNOSTIC</span>
                      </div>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center gap-10 group-hover:scale-105 transition-transform duration-700">
                      <div className="relative">
                         ...
                         <div className="w-20 h-20 rounded-[32px] bg-brand-light flex items-center justify-center border border-brand-dark/[0.02]">
                  ...
                  {/* Laser Line */}
                  <div className="absolute bottom-0 left-0 w-full h-[1.5px] bg-brand-primary/40 animate-scan z-40 opacity-40 shadow-[0_0_15px_#C2A36B]" />
                  
                  <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFileChange} />
               </div>

               {/* Sub-label under scanner to match Archive height visually */}
               <div className="absolute -bottom-8 right-10 opacity-10 hidden lg:block">
                  <p className="text-[6px] font-black uppercase tracking-[0.6em]">VERIFIED_INPUT_NODE</p>
               </div>
             </div>
          </div>
